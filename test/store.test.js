...
describe('Store', function () {
  context('API', () => {
    const storeName = 'foo'
    const passphrapse = 'password'
    const newPass = 'new password'

    it('Should fail to initialize if store name or passphrapse are not provided', async () => {
      let s
      ...
      try {
        s = new Store(undefined, passphrapse)
      } catch (error) {
        err = error
      }
    ...
    it('Should successfully initialize', async () => {
      let err = { message: '_ERROR_NOT_THROWN_' }
      try {
        const store = new Store(storeName, passphrapse)
        await store.init()
      } catch (error) {
        err = error
      }
    ...
    it('Should successfully updatePassphrase with the new password and retrieve saved data', async () => {
      ...
      await store.updatePassphrase(passphrapse, newPass)
